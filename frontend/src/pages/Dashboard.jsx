...
export default function Dashboard() {
  ...
  const fetchData = async (year = selectedYear, month = selectedMonth) => {
    try {
      ...
      const totalSpent = summaryRes.data.reduce(
      ...
      setChartData(
        summaryRes.data.map((item) => ({ name: item._id, value: item.total }))
      );
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 p-1">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="group bg-white hover:bg-gray-100 text-black px-5 py-2.5 rounded-xl font-bold transition flex items-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.15)]"
            >
              <Plus
                size={18}
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              {stats.budget === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Link
                    to="/profile"
                    className="inline-block bg-white text-indigo-600 px-6 py-3 rounded-xl font-bold hover:bg-indigo-50 transition"
                  >
                    Set Your Budget
                  </Link>
                </div>
                ...
                <>
                  ...
                  <div className="mt-8">
                    <div className="flex justify-between text-sm font-medium mb-2 text-indigo-100">
                      ...
                      <span>
                        {currencySymbol}
                        {stats.spent.toLocaleString()} Spent
                      </span>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-6">
            <div className="bg-gray-800/60 backdrop-blur-xl p-6 rounded-3xl border border-gray-700/50 hover:border-gray-600 transition group">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-xs font-medium text-gray-500 bg-gray-900/50 px-2 py-1 rounded-lg">
                  AI Forecast
                </span>
              </div>
              <h3 className="text-3xl font-bold text-white mb-1">
                {stats.forecast > 0 ? (
                  `${currencySymbol}${stats.forecast.toLocaleString()}`
                ) : (
          ...
          <div className="lg:col-span-2 bg-gray-800/40 backdrop-blur-md rounded-3xl border border-gray-700/50 p-6 shadow-xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
                <CreditCard size={20} className="text-gray-400" /> Recent
                Activity
              </h3>
              <Link
                to="/transactions"
            ...
            <div className="space-y-3">
              {recentTx.length > 0 ? (
                recentTx.map((tx, idx) => (
                  <div
                    key={idx}
                    className="group flex items-center justify-between p-4 rounded-2xl bg-gray-800/50 hover:bg-gray-700/50 border border-gray-700/50 hover:border-gray-600 transition duration-300"
                  >
        ...
        {loading ? (
          <div className="py-12 text-center space-y-4">
            ...
            <p className="text-indigo-300 font-medium">
              Analyzing data for {periodName}...
            </p>
          </div>
        ) : (
          <div className="space-y-6 text-gray-300 font-medium font-mono text-sm leading-7">
            {advice?.raw_advice ? (
              <div className="whitespace-pre-wrap">{advice.raw_advice}</div>
            ) : (
            ...
            {advice?.mock && (
              <div className="bg-yellow-500/10 border border-yellow-500/20 text-yellow-200 p-4 rounded-xl text-sm flex items-start gap-3 font-sans">
                ...
                <div>
                  ...
                  <span>
                    Please configure Gemini API Key for real insights.
                  </span>
                </div>
              </div>
            )}
      ...
      <div className="bg-gray-900 w-full max-w-md rounded-3xl border border-gray-700 shadow-2xl p-8 scale-100 animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 rounded-xl bg-indigo-600 text-white font-bold text-lg hover:bg-indigo-500 transition shadow-lg shadow-indigo-500/20 mt-4"
          >
